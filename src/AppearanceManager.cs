...
namespace MyChildCore.Utilities
{
    ...
    public static class AppearanceManager
    {
        ...
        /// <summary>
        /// 아기(Baby) 외형 적용 (모든 아기 공용 Body, 눈/스킨/헤어는 배우자 폴더)
        /// </summary>
        public static void ApplyBabyAppearance(Baby Baby, string spouseName)
        {
            if (Baby == null || string.IsNullOrEmpty(spouseName)) return;

            // 공용 바디(의상) 경로
            string bodyPath = "assets/clothes/Baby/Baby_Body.png";
            // 배우자별 헤어, 눈, 스킨 경로
            string hairPath = $"assets/{spouseName}/Baby/hair/{spouseName}_Baby_hair.png";
            string eyePath = $"assets/{spouseName}/Baby/eye/{spouseName}_Baby_eye.png";
            string skinPath = $"assets/{spouseName}/Baby/skin/{spouseName}_Baby_skin.png";

            // 적용 (순서: 바디 → 헤어 → 눈 → 스킨)
            ApplyIfExist(Baby, bodyPath, hairPath, eyePath, skinPath);
        }
