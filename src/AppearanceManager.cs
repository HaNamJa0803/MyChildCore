...
{
    /// <summary>
    /// Child appearance manager (안정성 & 하드코딩, SDV 1.6.15 모바일 최적)
    /// </summary>
    public static class AppearanceManager
    {
        ...
        /// 아기(Baby) 외형 적용
        /// </summary>
        public static void ApplyBabyAppearance(Child child, ModConfig config)
        {
            if (child == null || config == null || !config.EnableMod)
                return;

            ChildParts parts = PartsManager.GetPartsForBaby(child, config);
            if (parts == null)
                return;
        ...
        /// <summary>
        /// 유아(토들러) 외형 적용 (계절/성별/축제/잠옷 하드코딩 분기)
        /// </summary>
        public static void ApplyToddlerAppearance(Child child, ModConfig config)
        {
            if (child == null || config == null || !config.EnableMod)
                return;
        ...
        /// <summary>
        /// GMCM 옵션 변경 시 해당 배우자/성별 자녀만 즉시 적용 (ModConfig 인자 받음)
        /// </summary>
        public static void ApplyForGMCMChange(string spouse, bool isMale, ModConfig config)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (child.Age >= 1)
                    ApplyToddlerAppearance(child, config);
                else
                    ApplyBabyAppearance(child, config);
            }
        }
    }
}