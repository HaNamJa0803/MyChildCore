using System;
using System.IO;
using System.Linq; // ★ 부모 조회용 (FirstOrDefault)
using StardewValley;
...
namespace MyChildCore.Utilities
{
    ...
    public static class AppearanceManager
    {
        ...
        /// <summary>
        /// 자녀 인스턴스에서 배우자 이름 추출 (SDV 1.6.10+ : idOfParent를 통해 Farmer에서 이름을 조회)
        /// </summary>
        public static string GetSpouseName(Child child)
        {
            try
            {
                if (child?.spouse != null && !string.IsNullOrEmpty(child.spouse.Name.Values))
                    return child.spouse.Name;

                // SDV 1.6.10+: idOfParent로 Farmer 객체에서 이름 조회
