...
namespace MyChildCore
{
    ...
    {
        private static List<ChildData> _childCache = new();
        ...
        public static void RestoreBackup()
        {
            try
            {
                ...
                _childCache = JsonConvert.DeserializeObject<List<ChildData>>(data) ?? new();
        ...
        {
            _childCache = new List<ChildData>(anySource ?? new());
            BackupCache();
            ResourceManager.InvalidateAllChildrenSprites();
        ...
        public static List<ChildData> GetBabies()   => _childCache?.FindAll(c => c.Age == 0) ?? new();
        public static List<ChildData> GetToddlers() => _childCache?.FindAll(c => c.Age == 1) ?? new();
        public static List<ChildData> GetByGender(string gender) => _childCache?.FindAll(c => c.Gender == gender) ?? new();
        public static List<ChildData> GetBySpouse(string spouse) => _childCache?.FindAll(c => c.Spouse == spouse) ?? new();
