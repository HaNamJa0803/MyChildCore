...
namespace MyChildCore
{
    ...
    /// + 실시간 연동/알림(Observer) 이벤트 지원 (즉시 감지 완비)
    /// </summary>
    public static class CacheManager
    {
        ...
        public static void SaveCacheToSaveData(IModHelper helper)
        {
            try
            {
                ...
                // 저장 후 자동 외부 백업도 함께 실행
                BackupCacheToExternal();
                NotifyCacheChanged();
            }
            catch (Exception ex)
            {
        ...
        public static void BackupCacheToExternal()
        {
            try
            {
                ...
                File.WriteAllText(BackupPath, Newtonsoft.Json.JsonConvert.SerializeObject(_childCache, Newtonsoft.Json.Formatting.Indented));
        ...
        public static void SetChildCache(IModHelper helper, List<ChildData> list)
        {
            ...
            SaveCacheToSaveData(helper); // 자동 백업까지
        ...
        {
            _childCache = new List<ChildData>();
            SaveCacheToSaveData(helper);  // 외부 백업도 자동!
            ResourceManager.InvalidateAllChildrenSprites();
