...
namespace MyChildCore
{
    ...
    public static class ChildManager
    {
        ...
        public static List<NPC> GetAllSpouses()
        {
            ...
            // 2. 플레이어 직접 추가 (NPC 캐스팅 주의)
            if (Game1.player is NPC playerNpc)
                all.Add(playerNpc);
            // 또는 아래처럼 플레이어를 별도 취급 (플레이어는 NPC 아님)
            // 실제 플레이어 이름으로 다루려면
            // all.Add(new NPC(Game1.player.Name, Vector2.Zero, 2, 2, 2, false, null, null));

            return all;
        }

        /// <summary>
        /// 모든 자녀 객체 리스트 (모든 지역 순회)
        /// </summary>
        ...
        {
            // 플레이어는 NPC 타입이 아니므로, 실제 결혼가능/결혼상태 NPC의 이름만 반환
