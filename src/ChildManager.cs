    ...
    public static class ChildManager
    {
        ...
        public static List<Child> GetAllChildren()
        {
            try
            {
                ...
                foreach (var farmer in Game1.getAllFarmers())
                {
                    ...
                    var locations = new[] { farmer.homeLocation, farmer.currentLocation }
                        .Where(l => l != null)
                        .Distinct();
        ...
        public static void OverwriteCacheFromGame()
        {
            ...
            foreach (var child in children)
            {
                ...
                {
                    CustomLogger.Error($"[ChildManager] FromChild 변환 예외: {child?.Name ?? "null"}: {ex.Message}");
                }
            }
        ...
        public static void ForceAllChildrenAppearance(ModConfig config)
        {
            ...
            foreach (var child in children)
            {
                ...
                {
                    CustomLogger.Error($"[ChildManager] ForceAllChildrenAppearance 예외: {child?.Name ?? "null"}: {ex.Message}");
                }
            }
        }
    }
