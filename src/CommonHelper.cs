...
internal static class CommonHelper
{
	...
	{
		IEnumerable<GameLocation> enumerable = Game1.locations.Concat(from location in Game1.locations
		...
		{
			enumerable = enumerable.Concat((IEnumerable<GameLocation>)MineShaft.activeMines).Concat((IEnumerable<GameLocation>)VolcanoDungeon.activeLevels);
		}
		return enumerable;
	}
	...
	public static Vector2 GetPositionFromAnchor(int offsetX, int offsetY, int width, int height, PositionAnchor anchor, bool? uiScale = null, int zoom = 4)
	{
		...
		Rectangle val = ((uiScale ?? Game1.uiMode) ? Game1.uiViewport : Game1.viewport);
		if ((anchor == PositionAnchor.BottomRight || anchor == PositionAnchor.TopRight) ? true : false)
		{
			offsetX = ((Rectangle)(ref val)).Width - width * zoom - offsetX;
		}
		if ((uint)anchor <= 1u)
		{
			offsetY = ((Rectangle)(ref val)).Height - height * zoom - offsetY;
		}
