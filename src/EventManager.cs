...
namespace MyChildCore
{
    ...
    public static class EventManager
    {
        ...
        public static void RegisterEvents(IModHelper helper)
        {
            ...
            // 실시간 데이터 변경(캐시, GMCM 등) 알림 구독 (연결성 보완)
            CacheManager.OnChildCacheChanged += (childList) =>
            {
                ForceAllChildrenAppearance(Config); // 자녀 데이터가 바뀌면 즉시 외형 전체 동기화
            };
            // GMCMManager.OnOptionChanged += () => ForceAllChildrenAppearance(Config); // 필요시 GMCM 등도 연동
        }
        ...
        public static string BuildSpritePath(Child child, ChildParts parts)
        {
            ...
            string skin = parts?.SkinName ?? "body";
            string suffix = parts?.IsDarkSkin == true ? "_dark" : "";
