    ...
    public static class GMCMManager
    {
        ...
        public static void RegisterGMCM(IModHelper helper, Mod mod)
        {
            ...
            foreach (var spouse in DropdownConfig.SpouseNames)
            {
                ...
                // 1) [기본값으로 초기화] 옵션 (옵션 상단에 배치)
                gmcm.AddTextOption(
                    mod: mod.ModManifest,
                    name: () => "[기본값으로 초기화]",
                    tooltip: () => "이걸 클릭하면 이 배우자의 자녀 설정이 모두 기본값으로 초기화됩니다.",
                    getValue: () => "기본값 초기화",
                    setValue: v =>
                    {
