        ...
        public static void RegisterGMCM(IModHelper helper, Mod mod)
        {
            ...
            foreach (var spouse in DropdownConfig.SpouseNames)
            {
                ...
                gmcm.AddTextOption(
                    ...
                    allowedValues: () =>
                    {
                        if (DropdownConfig.BabyHairColorOptions.TryGetValue(Config.SpouseConfigs[spouse].BabyHairType, out var list))
                            return list.ToArray();
                        if (DropdownConfig.BabyHairColorOptions.TryGetValue("Default", out var def))
                            return def.ToArray();
