...
{
    // Observer 이벤트/콜백 (옵션값 변경시 실시간 연결용)
    public static class GMCMObserver
    {
        // 옵션값 변경시 호출될 이벤트 (외부 매니저에서 구독)
    ...
    public static class GMCMOptionsMenus
    {
        public static readonly List<GMCMOption> GMCMOptions = new()
        {
            ...
            new GMCMOption
            {
                ...
                SetValue = (s, v) => {
                    DropdownConfig.EnableMod = v == "True";
                    GMCMObserver.Notify(s, "EnableMod", v);
                },
            ...
            new GMCMOption
            {
                ...
                SetValue = (s, v) => {
                    DropdownConfig.EnablePajama = v == "True";
                    GMCMObserver.Notify(s, "EnablePajama", v);
                },
                AllowedValues = s => new[] { "True", "False" }
            },
            new GMCMOption
            {
                ...
                SetValue = (s, v) => {
                    DropdownConfig.EnableFestival = v == "True";
                    GMCMObserver.Notify(s, "EnableFestival", v);
                },
                AllowedValues = s => new[] { "True", "False" }
            },
            // ====== 아기(공용) ======
            ...
            // ... (이하 전체 옵션 모두 같은 방식으로 SetValue에 GMCMObserver.Notify 추가)
            // ... 모든 AllowedValues, GetValue, SetValue는 기존 코드 동일하게 유지
        };

        // (참고) 옵션 전체 동기화 예시 (옵션이 변경될 때마다)
        static GMCMOptionsMenus()
        {
            // 외부 매니저에서 Observer 구독
            GMCMObserver.OnOptionChanged += (spouse, option, value) =>
            {
                // 실시간 동기화 코드 예시
                // 예시: 변경된 spouse의 외형/캐시/리소스/저장 자동 반영
                ...
                if (config.SpouseConfigs.ContainsKey(spouse))
                {
                    ...
                    // 2. 저장/캐시 최신화
                    DataManager.ManualSaveFromCache();

                    // 3. 필요시 UI/알림/리소스 무효화 등 추가
