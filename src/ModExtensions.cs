...
internal static class ModExtensions
{
	public static TInterface GetApi<TInterface>(this IModRegistry modRegistry, string uniqueId, string label, string minVersion, IMonitor monitor) where TInterface : class
	{
		...
		IManifest val = ((obj != null) ? obj.Manifest : null);
		if (val == null)
		{
			return null;
		}
		if (val.Version.IsOlderThan(minVersion))
		{
			monitor.Log($"Detected {label} {val.Version}, but need {minVersion} or later. Disabled integration with this mod.", (LogLevel)3);
			return null;
		}
