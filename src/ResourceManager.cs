...
{
    /// <summary>
    /// 자녀 리소스(스프라이트 등) 실시간 캐시 무효화 및 직접 교체 매니저 (SMAPI 6.0 기준, Observer 포함)
    /// </summary>
    public static class ResourceManager
    {
        /// <summary>
        /// 캐릭터 스프라이트 캐시 무효화 + 즉시 이미지 할당 (PatchImage)
        /// </summary>
        public static void ApplyChildSprite(string childName, Texture2D sprite)
        {
            ...
            try
            {
                ...
                // 3. 실시간 동기화 이벤트 알림(즉시 감지!)
                NotifySpriteChanged(childName);
            }
            catch (Exception ex)
            {
                CustomLogger.Warn($"[ResourceManager] {childName} 캐시 무효화/할당 실패! Error: {ex.Message}");
            }
        }
