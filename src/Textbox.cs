...
internal class Textbox : Element, IKeyboardSubscriber
{
	...
	public override void Update(bool isOffScreen = false)
	{
		...
		if (base.ClickGestured && Callback != null)
		{
			Selected = base.Hover;
		}
	}
	...
	public override void Draw(SpriteBatch b)
	{
		...
		if (!IsHidden())
		{
			b.Draw(Tex, base.Position, Color.White);
			string text = String;
			Vector2 val = Font.MeasureString(text);
			while (val.X > 192f)
			{
				text = text.Substring(1);
				val = Font.MeasureString(text);
			}
			if (DateTime.UtcNow.Millisecond % 1000 >= 500 && Selected)
			{
				b.Draw(Game1.staminaRect, new Rectangle((int)base.Position.X + 16 + (int)val.X + 2, (int)base.Position.Y + 8, 4, 32), Game1.textColor);
			}
			b.DrawString(Font, text, base.Position + new Vector2(16f, 12f), Game1.textColor);
		}
	}
